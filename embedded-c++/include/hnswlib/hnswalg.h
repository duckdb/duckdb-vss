...
//                    if (candidate_id == 0) continue;
    ...
    */
    ...
    searchKnn(const void *query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const override {
        ...
        // Populate metrics
        result.visited_members = metric_hops;
        result.computed_distances = metric_distance_computations;
    
        while (!top_candidates.empty()) {
            std::pair<dist_t, tableint> rez = top_candidates.top();
            result.neighbors.push(std::pair<dist_t, labeltype>(rez.first, getExternalLabel(rez.second)));
            top_candidates.pop();
        }
    
        result.count = result.neighbors.size();
        result.success = true;
