...
class AlgorithmInterface {
 public:
    struct KnnSearchResult {
        // Priority queue of results (distance, label)
        std::priority_queue<std::pair<dist_t, labeltype>> neighbors;
        ...
        /** @brief Indicates if the search was successful. */
        bool success{false};

        // Conversion operator to maintain backward compatibility
        operator std::priority_queue<std::pair<dist_t, labeltype>>() const {
            return neighbors;
        }
    };
